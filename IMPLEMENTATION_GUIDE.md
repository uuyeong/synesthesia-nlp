...
> **이 함수가 1단계의 핵심.** B의 후보 단어 9,770개 사전 계산이 이 함수에 의존.
...
- 개행 기준으로 split → 9,770개 단어 리스트

#### ② `load_candidate_vectors(candidate_words, tokenizer, model, mean_vec)` → `np.ndarray (9770, 768)`

```
캐시 로직:
    data/candidate_vectors.npy 있으면 → np.load()로 바로 사용
    없으면 → get_bert_vector()로 9,770개 계산 후 저장
...
B가 9,770개 단어 벡터 계산 완료 후 팀원들에게 공유.  
